...
class SpeechLMPreprocessor(AbsPreprocessor):
    ...
    def __init__(
        ...
        # speech_ssl_encoder
        speech_ssl_encoder = speech_ssl_encoder_conf.get("hf_tag", None)
        if speech_ssl_encoder is not None:
            if speech_ssl_encoder == "Qwen/Qwen2-Audio-7B":
                try:
                    from transformers import AutoFeatureExtractor
                except ImportError:
                    raise ImportError(f"Cannot import Qwen2AudioEncoder object")
                self.speech_ssl_processor = AutoFeatureExtractor.from_pretrained(
    ...
    def __call__(
        ...
        for idx, data_tuple in enumerate(data_tuples):
            ...
            if conti_feat is not None:
                for c_feat, c_len in zip(conti_feat, conti_len):
                    conti_feats.append([c_feat, modality, idx, c_len])
        ...
        )[0] + 1 # shift by one due to modality token
        for conti_feat, modality, idx, conti_len in conti_feats:
            start = modality_identifier_indices[idx]
            new_conti_feats.append((conti_feat, modality, start, conti_len))
            modality_identifier_indices[idx] += conti_len
    ...
    def modality_specific_processing(self, value, modality, cache, end_tok=None):
        ...
        # Other discrete modalities
        ...
        elif modality in ["speech_ssl_encoder"]:
            fs, array = value
            if not fs == self.speech_ssl_sampling_rate:
                logging.warning(
                    f"find audio with sampling rate {fs}. Will do re-sample it"
                )
            ...
            output = self.speech_ssl_processor(
                ...
                sampling_rate=self.speech_ssl_sampling_rate
            )['input_features']

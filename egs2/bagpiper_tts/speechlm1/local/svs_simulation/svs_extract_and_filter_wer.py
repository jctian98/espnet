...
"""Stage 1: Extract lyrics from rich captions via LLM, filter by WER ≤ 20%.

The reference transcript is the Whisper output from Stage 0 (not ground truth),
since no ground truth lyrics are available for singing data.  A lenient WER
threshold of 20% is used to retain more examples while still ensuring the
caption contains roughly the correct lyrics.
"""
...
# =============================================================================
# Prompts for lyrics extraction from singing captions
# =============================================================================
PROMPTS_BY_VERSION = {
    "v1": {
        "system": (
            "You are a precise text extraction assistant. Given a rich audio "
            "caption that describes a singing recording, extract the EXACT "
            "lyrics sung by the vocalist. Output ONLY the verbatim lyrics "
            "with no quotes, no explanation, and no additional formatting. "
            "Include all lines of lyrics that are described or quoted in the "
            "caption. If no lyrics are described, output exactly: NO_LYRICS"
        ),
        "user": (
            "Extract the exact lyrics from this singing audio caption:\n\n"
            "{caption}"
        ),
    },
}
...
def load_input_data(input_file: str) -> List[Dict[str, Any]]:
    """Load stage0 JSONL, skipping samples with has_whisper_lyrics=False."""
    samples = []
    skipped_no_lyrics = 0
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                ...
                if not record.get("has_whisper_lyrics", True):
                    skipped_no_lyrics += 1
                    continue
                samples.append(record)
    if skipped_no_lyrics > 0:
        print(f"Skipped {skipped_no_lyrics} samples with has_whisper_lyrics=False")
    return samples
...
def build_extraction_query(
    ...
    """Build a query to extract lyrics from the singing caption."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage 1: Extract lyrics from captions and filter by WER."
    )
    parser.add_argument(
        "--input_file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.20,
        help="Maximum WER to keep a sample (default: 0.20).",
    )

...
"""Stage 0: Transcribe singing audio using Qwen3-ASR via vLLM.
...
def process_transcription_result(
    ...
    Samples where lyrics cannot be extracted are still saved with
    ``has_whisper_lyrics=False`` and an empty ``whisper_lyrics`` field.
    """
    response = result.get("response")
    metadata = result.get("metadata", {})

    has_whisper_lyrics = True
    lyrics = ""

    if response is None:
        _drop_stats["no_response"] += 1
        has_whisper_lyrics = False
    else:
    ...
    output = {
        ...
        "has_whisper_lyrics": has_whisper_lyrics,
        "idx": result["idx"],
    }
    return output
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage 0: Transcribe singing audio with Qwen3-ASR."
    )
    parser.add_argument(
        "--input_file",
        type=str,

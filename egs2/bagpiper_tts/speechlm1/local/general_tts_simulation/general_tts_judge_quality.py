...
"""Stage 4: LLM-as-judge quality validation with application-aware dimensions."""
...
SYSTEM_PROMPT = (
    "You are a STRICT quality judge for text-to-speech (TTS) "
    "training data. Your task is to critically evaluate TTS training "
...
Critically evaluate a text-to-speech (TTS) training example. \
...
- application_appropriateness: Is the user request natural and appropriate \
for the claimed application category? Would a real person write this request \
for this specific use case?
  * Flaws: request doesn't fit the category, unnatural phrasing for the \
application, forced or contrived connection to the category
- voice_attribute_match: Does the caption describe the requested voice \
attributes (tone, pace, emotion, accent)?
  * Flaws: wrong emotion/pace, mismatched gender or age
- transcription_faithfulness: Is the requested spoken text accurately \
reflected?
  * Flaws: text in caption differs from request, words omitted or changed
...
- reasoning_coherence: Does the TTS reasoning flow logically?
  * Flaws: contradictions, jumps in logic, delivery advice that contradicts \
the speaking context
- cot_caption_consistency: Does the CoT plan align with what the caption \
describes?
  * Flaws: CoT mentions prosodic features or delivery choices not present \
in caption, contradictions between planned delivery and described performance
- application_cot_alignment: Does the CoT reasoning address application-\
specific delivery conventions? Does the reasoning demonstrate understanding \
of what the application category demands?
  * Flaws: CoT ignores the application context, generic reasoning that could \
apply to any category, no mention of application-specific conventions
...
**Rich Caption (target speech audio description):**
{caption}"""

EXPECTED_DIMENSIONS = [
    "application_appropriateness",
    "voice_attribute_match",
    "transcription_faithfulness",
    "structure_completeness",
    "reasoning_coherence",
    "cot_caption_consistency",
    "application_cot_alignment",
]
...
def _validate_response_structure(data: Dict[str, Any]) -> bool:
    for dim in EXPECTED_DIMENSIONS:
        if dim not in data:
            key = f"missing_dim:{dim}|actual={list(data.keys())}"
...
def build_judge_query(sample: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Build a quality-judge query for a TTS sample."""
...
def make_process_judge_result(
    ...
    def process_judge_result(
        ...
        return {
            ...
            "application_category": metadata.get(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage 4: LLM-as-judge quality validation for TTS."
    )
    parser.add_argument(
        "--input_file",
        type=str,

...
#
# General Text-to-Speech (TTS) Data Simulation Pipeline
# Transforms (speech audio + rich caption) into dialogue-format training data:
#   System prompt -> User TTS request -> CoT -> Rich Caption -> Audio
#
# Stage 0: Qwen3-ASR transcription to obtain text reference
# Stage 1: LLM extracts spoken text from caption; filter by WER <= 10%
# Stage 2: Generate TTS user requests
...
input_file=/mnt/home/jinchuat-andr-d6b58f/jinchuat/data/raw_sft/speech_2m/metadata.jsonl
...
# WER threshold for Stage 1 (stricter for speech data)
wer_threshold=0.10
...
# ---------------------------------------------------------------------------
# Stage 0: Whisper transcription to get text reference
# ---------------------------------------------------------------------------
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    log "Stage 0: Qwen3-ASR transcription of speech audio"

    python3 local/general_tts_simulation/general_tts_whisper_transcribe.py \
...
# ---------------------------------------------------------------------------
# Stage 1: Extract spoken text from caption; filter by WER <= wer_threshold
# ---------------------------------------------------------------------------
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "Stage 1: Extracting spoken text from captions (WER <= ${wer_threshold})"
    ...
    python3 local/general_tts_simulation/general_tts_extract_and_filter_wer.py \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "Stage 2: Generating TTS user requests"
    ...
    python3 local/general_tts_simulation/general_tts_generate_user_requests.py \
    ...
    if [ -f "${stage2_output}" ]; then
        ...
        log "Stage 2 completed: ${count} TTS user requests generated"
    fi
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    python3 local/general_tts_simulation/general_tts_generate_cot.py \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    python3 local/general_tts_simulation/general_tts_judge_quality.py \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    log "Stage 5: Assembling TTS dialogues"
    ...
    python3 local/general_tts_simulation/general_tts_assemble_dialogue.py \
    ...
    if [ -f "${stage5_output}" ]; then
        ...
        log "Stage 5 completed: ${count} TTS dialogues assembled"

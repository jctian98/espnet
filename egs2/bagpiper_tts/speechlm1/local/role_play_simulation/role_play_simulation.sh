...
#
# Role-Play TTS Data Simulation Pipeline
# Transforms rich captions into dialogue-format training data where the user
# provides a fictional CHARACTER PERSONA and the model infers voice attributes
# from that persona during chain-of-thought reasoning.
#
# Input: JSONL from general TTS stage1 (with example_id, caption, whisper_lyrics, etc.)
# Output: System prompt -> Persona request -> CoT (with voice inference) -> Caption -> Audio
...
# Stage 1 persona score threshold (1-5)
stage1_min_score=5
...
output_dir="data/role_play_tts/${version}"
...
# Stage 1: Filter captions for persona suitability
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "Stage 1: Filtering captions for persona suitability"
    ...
    python3 local/role_play_simulation/rp_filter_captions.py \
    ...
    if [ -f "${stage1_output}" ]; then
        ...
        log "Stage 1 completed: ${count} samples passed persona suitability filter"
    fi
fi

# Stage 2: Generate persona-based user requests
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "Stage 2: Generating persona-based user requests"
    ...
    python3 local/role_play_simulation/rp_generate_user_requests.py \
    ...
    if [ -f "${stage2_output}" ]; then
        ...
        log "Stage 2 completed: ${count} persona requests generated"
    fi
fi

# Stage 3: Generate chain-of-thought with voice inference
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    log "Stage 3: Generating CoT with voice inference from persona"
    ...
    python3 local/role_play_simulation/rp_generate_cot.py \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    python3 local/role_play_simulation/rp_judge_quality.py \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    python3 local/role_play_simulation/rp_assemble_dialogue.py \

...
"""Stage 4: LLM-as-judge quality validation for role-play TTS examples.

Uses 6 role-play-specific dimensions to evaluate quality:
1. persona_richness — Is the persona detailed, specific, and believable?
2. persona_voice_consistency — Do inferred voice features logically follow
   from the persona?
3. voice_caption_match — Do CoT-inferred voice features match the actual
   voice in the caption?
4. structure_completeness — Are all 3 CoT parts present and adequate?
5. reasoning_coherence — Does reasoning flow logically?
6. cot_caption_consistency — Does CoT align with the caption?
"""
...
SYSTEM_PROMPT = (
    "You are a STRICT quality judge for role-play text-to-speech training "
    "data. Your task is to critically evaluate training examples where the "
    "user provides a CHARACTER PERSONA and text to speak, and the system "
    "infers voice characteristics from the persona.\n\n"
...
Critically evaluate a role-play TTS training example. \
...
- persona_richness: Is the persona detailed, specific, and believable?
  * Flaws: generic description, lacks specificity, unrealistic character, \
too brief
- persona_voice_consistency: Do inferred voice features in Part B logically \
follow from the persona in Part A?
  * Flaws: voice features don't match character traits, unjustified \
inferences, missing reasoning links
- voice_caption_match: Do CoT-inferred voice features match the actual \
voice in the caption?
  * Flaws: inferred gender/age/accent contradicts caption, major voice \
...
Output JSON only:
{{"persona_richness": {{"score": 4}}, \
"persona_voice_consistency": {{"flaws": "...", "score": 3}}, \
"voice_caption_match": {{"score": 4}}, \
"structure_completeness": {{"score": 4}}, \
"reasoning_coherence": {{"score": 4}}, \
"cot_caption_consistency": {{"flaws": "...", "score": 3}}}}

## Role-Play TTS Training Example to Evaluate

**User Request (persona + text):**
{user_request}

**Thinking Trace (assistant's reasoning with voice inference):**
{cot}

**Rich Caption (target audio description):**
{caption}

**Whisper Lyrics (actual spoken words for reference):**
{whisper_lyrics}"""
    ...
    with open(filtered_file, "w", encoding="utf-8") as out_f:
        with open(judge_file, "r", encoding="utf-8") as in_f:
            for line in in_f:
                ...
                # Recompute pass/fail using current CLI thresholds
                # (not the pre-baked overall_pass from a previous run)
                scores = judge_result.get("scores", {})
                _, _, current_pass = compute_overall_score(
                    scores, args.min_score, args.avg_score
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="LLM-as-judge quality validation for role-play TTS."
    )
    parser.add_argument(
        "--input_file",
        type=str,

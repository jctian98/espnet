...
"""Stage 5: Assemble filtered ITS examples into dialogue format."""
...
SYSTEM_PROMPTS_BY_VERSION = {
    "v1": (
        "You are an advanced intention-to-speech synthesis system. "
        "Users describe what they want to communicate — the purpose, "
        "occasion, and desired voice characteristics — without providing "
        "exact words. You infer the appropriate wording and generate "
        "natural, expressive speech that fulfills their communicative intent."
    ),
}
...
def create_example_id(sample: Dict[str, Any]) -> str:
    ...
    return f"intention_tts_{dataset}_{example_id}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Assemble ITS dialogue from filtered examples."
    )
    parser.add_argument(
        "--input_file",
        type=str,

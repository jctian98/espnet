...
"""Stage 4: LLM-as-judge quality validation for intention-to-speech examples.

Uses 6 ITS-specific dimensions to evaluate quality:
1. request_naturalness — Does the request sound like a genuine human intention?
2. no_transcription_leakage — Does the request avoid revealing transcription?
3. inference_quality — Does Part B infer text that matches the audio?
...
SYSTEM_PROMPT = (
    "You are a STRICT quality judge for intention-to-speech training data. "
    "Your task is to critically evaluate training examples where the user "
    "provides a communicative INTENTION (not exact words) and the system "
    "must infer what to say.\n\n"
...
Critically evaluate an intention-to-speech training example. \
...
- request_naturalness: Does the request sound like a genuine human intention?
  * Flaws: robotic phrasing, unnatural formality, unclear purpose
- no_transcription_leakage: Does the request avoid revealing transcription content?
  * Flaws: specific phrases from the transcript appear in the request, \
direct quoting, close paraphrasing
- inference_quality: Does Part B infer text that matches the spoken audio?
  * Flaws: inferred text differs from whisper_lyrics, wrong meaning, \
missing content
...
Output JSON only:
{{"request_naturalness": {{"score": 4}}, \
"no_transcription_leakage": {{"score": 5}}, \
"inference_quality": {{"flaws": "...", "score": 3}}, \
"structure_completeness": {{"score": 4}}, \
"reasoning_coherence": {{"score": 4}}, \
"cot_caption_consistency": {{"flaws": "...", "score": 3}}}}

## ITS Training Example to Evaluate

**User Intention Request:**
{user_request}

**Thinking Trace (assistant's reasoning with inferred transcription):**
{cot}
...
def main():
    parser = argparse.ArgumentParser(
        description="LLM-as-judge quality validation for ITS examples."
    )
    parser.add_argument(
        "--input_file",
        type=str,

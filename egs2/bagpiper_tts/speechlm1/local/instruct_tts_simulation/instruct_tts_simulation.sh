...
#
# Instruct TTS Data Simulation Pipeline
...
# Data root for LibriTTS-R
data_root="/mnt/home/jinchuat-andr-d6b58f/jinchuat/espnet_speechlm_tts/egs2/libritts_r/tts1"
# subset="train-clean-100,train-clean-360,train-other-500,dev-clean,test-clean"
subset="test-clean"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    python3 local/instruct_tts_simulation/tts_extract_and_filter_wer.py \
        --data_root "${data_root}" \
        --subsets "${subset}" \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    python3 local/instruct_tts_simulation/tts_generate_user_requests.py \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    python3 local/instruct_tts_simulation/tts_generate_cot.py \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    python3 local/instruct_tts_simulation/tts_judge_quality.py \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    python3 local/instruct_tts_simulation/tts_assemble_dialogue.py \

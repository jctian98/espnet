...
def load_jsonl_format(
    metadata_jsonl: str, transcription_jsonl: str
) -> List[Dict[str, Any]]:
    """Load data from metadata + transcription JSONL files.

    Joins on audio_path. Strips leading '#' from transcriptions.
    Discards entries with gender variants ({F# or {M# in transcription).

    Returns list of sample dicts with the same schema as load_all_subsets:
    utt_id, text, audio_path, speaker_id, caption, duration, subset.
    """
    # Load transcriptions keyed by audio_path
    transcriptions = {}
    with open(transcription_jsonl, "r", encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            record = json.loads(line)
            transcriptions[record["audio_path"]] = record["transcription"]

    print(f"Loaded {len(transcriptions)} transcriptions from {transcription_jsonl}")

    # Load metadata and join with transcriptions
    samples = []
    skipped_no_transcription = 0
    skipped_gender_variant = 0
    ...
    with open(metadata_jsonl, "r", encoding="utf-8") as f:
        for line in f:
            ...
            transcription = transcriptions[audio_path]

            # Discard entries with gender variants
            if "{F#" in transcription or "{M#" in transcription:
                skipped_gender_variant += 1
                continue

            # Strip leading '#' from transcription
            ...
            # Generate utt_id from dataset + filename stem
            dataset = record.get("dataset", "unknown")
            filename_stem = os.path.splitext(os.path.basename(audio_path))[0]
            utt_id = f"{dataset}_{filename_stem}"

            # Use qwen_caption, fallback to gemini_caption
            caption = record.get("qwen_caption", "") or record.get(
                "gemini_caption", ""
            )
    ...
    print(
        f"Loaded {len(samples)} samples from {metadata_jsonl} "
        f"(skipped: {skipped_no_transcription} no transcription, "
        f"{skipped_gender_variant} gender variants)"
    )
...
async def main_async(args):
    ...
    if args.data_root is not None:
        ...
        print(f"  Metadata: {args.metadata_jsonl}")
        print(f"  Transcription: {args.transcription_jsonl}")
        samples = load_jsonl_format(args.metadata_jsonl, args.transcription_jsonl)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to metadata JSONL file (alternative to --data_root).",
    )
    parser.add_argument(
        "--transcription_jsonl",
        type=str,
        default=None,
        help="Path to transcription JSONL file (required with --metadata_jsonl).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    # Validate input mode
    has_jsonl = args.metadata_jsonl is not None or args.transcription_jsonl is not None
    has_data_root = args.data_root is not None

    if has_jsonl and has_data_root:
        parser.error(
            "Cannot use both --data_root and --metadata_jsonl/--transcription_jsonl"
        )
    if not has_jsonl and not has_data_root:
        parser.error(
            "Must provide either --data_root or "
            "(--metadata_jsonl and --transcription_jsonl)"
        )
    if has_jsonl:
        if args.metadata_jsonl is None or args.transcription_jsonl is None:
            parser.error(
                "Both --metadata_jsonl and --transcription_jsonl are required together"
            )

...
"""Stage 5: Assemble filtered multi-talker TTS examples into dialogue format."""
...
SYSTEM_PROMPTS_BY_VERSION = {
    "v1": "You are a multi-talker text-to-speech system.",
}
...
def create_example_id(sample: Dict[str, Any]) -> str:
    ...
    return f"multi_talker_tts_{utt_id}"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Assemble multi-talker TTS dialogue from filtered examples."
        )
    )
    parser.add_argument(
        "--input_file",

...
class Qwen2AudioReader(BaseFeatureReader):
    def __init__(
        self,
        checkpoint,
        fs: Union[int, str] = 16000,
    ...
    @torch.no_grad()
    def get_feats(self, data, data_lens, ref_len = None):
        # Preprocessor accepts the numpy.array list
        data_raw, bucket_sizes = [], []
        for idx, wav in enumerate(data.cpu().numpy()):
            length = data_lens[idx].clone()
            count = 0
            while length > 0:
                clip = wav[:min(length, self.feature_extractor.n_samples)]
                data_raw.append(clip)
                length -= self.feature_extractor.n_samples
                count += 1
        ...
        output = self.model(input_features=output['input_features'])
        ...
        for bucket_size in bucket_sizes:
            bucket = []
            while bucket_size > 0:
                bucket.append(output.pop(0))
                bucket_size -= 1
            bucket = torch.cat(bucket, dim=0)
            spliced_output.append(bucket)
        
        hop_size = self.feature_extractor.hop_length * 4 # 4 times downsampling
        data_lens = data_lens // hop_size + (data_lens % hop_size != 0).int()

        # To double the audio feats length for 50 Hz. The very temporary code.
        spliced_output = [torch.repeat_interleave(x, 2, dim=0) for x in spliced_output]
        data_lens = data_lens * 2

        return spliced_output, data_lens
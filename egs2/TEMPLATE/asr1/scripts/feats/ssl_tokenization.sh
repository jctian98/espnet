...
elif [ "${ssl_choice}" == "espnet_hubert" ] || [ "${ssl_choice}" == "qwen2audio" ]; then
    if [ ! -e ${checkpoint_path} ] || [ ! -e ${kmeans_path} ]; then
        if [ ! -e ${checkpoint_path} ]; then echo ${checkpoint_path}; fi
        if [ ! -e ${kmeans_path} ]; then echo ${kmeans_path}; fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    elif [ "${ssl_choice}" == "qwen2audio" ]; then
        feature_conf="{type=qwen2audio,conf={checkpoint=${checkpoint_path}}}"

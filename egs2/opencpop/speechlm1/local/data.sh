...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # convert the score separate json files to an uniform text file
    for dset in ${train_set} ${train_dev} ${test_set}; do
        python utils/data/internal/convert_score.py --data_folder data/${dset}
    done
fi
